 ...
 *              elow >= usage comparison. Returning false falls through to
...
static inline unsigned long bpf_memcg_charged(struct mem_cgroup *memcg)
{
	unsigned int ret;
	 ...
	 */
	ret = BPF_MEMCG_CALL(memcg, memcg_charged, 0U);
	return msecs_to_jiffies(ret);
}

static inline void bpf_memcg_uncharged(struct mem_cgroup *memcg)
{
	BPF_MEMCG_CALL_VOID(memcg, memcg_uncharged);
}
...
bpf_memcg_charged(struct mem_cgroup *memcg) { return 0; }
static inline bool
